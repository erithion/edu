...
parser = ArgumentParser(description="Regression with Gauss-Newton method")
...
#    pdb.set_trace()
    ...
    while True:
        ...
        beta_ = beta + fraction * shift
        J = J_f(beta_, X)
        y_ = f(beta_, X)
        dif = Y - y_
        cost_f = dif.T.dot(dif)

        # divergence ?
        if (sum.size > 1 and cost_f > sum[-1]):
#            pdb.set_trace()
            fraction /= 2
            continue

        pinv = np.linalg.pinv(J.T.dot(J))
        shift = pinv.dot(J.T).T.dot(dif)
        # reset fraction
        fraction = 1
...
beta = np.ones(N)    
beta, profile = gauss_newt(beta, X, Y, J, f)        
